#!/usr/bin/env python3
"""
Visualize Wanda Full Matrix K-sweep Results and Compare with 128-feature Subset
...
def create_comparison_heatmap(subset_results: dict, full_results: dict, output_dir: Path):
    ...
    plt.suptitle('Wanda: Local vs Global Clustering Structure', fontsize=14, y=0.98)
    plt.tight_layout()

    output_file = output_dir / 'wanda_subset_vs_full_comparison.png'
...
def create_ratio_difference_heatmap(subset_results: dict, full_results: dict, output_dir: Path):
    ...
    ax.set_title('Wanda: Local vs Global Clustering Strength\n(Ratio > 1: Subset stronger | Ratio < 1: Full matrix stronger)',
                fontsize=13, pad=20)
    ...
    output_file = output_dir / 'wanda_subset_vs_full_ratio.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
...
def create_full_matrix_line_plot(full_results: dict, output_dir: Path):
    ...
    ax.set_title('Wanda Full Matrix: Separation Ratio vs K Value\n(Global Clustering Structure)',
                fontsize=14, pad=20)
    ...
    output_file = output_dir / 'wanda_full_matrix_line_plot.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
...
def create_cluster_size_distribution(full_results: dict, output_dir: Path):
    ...
    ax.set_xlabel('Number of Clusters (k)', fontsize=12)
    ax.set_ylabel('Projection Type', fontsize=12)
    ax.set_title('Wanda Full Matrix: Cluster Size Uniformity\n(Lower = more uniform, Higher = more skewed)',
                fontsize=13, pad=20)
    ...
    output_file = output_dir / 'wanda_full_matrix_cluster_uniformity.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
...
def main():
    """Generate all visualizations for Wanda full matrix results."""
    base_dir = Path(__file__).resolve().parent.parent.parent

    # Load both datasets
    subset_file = base_dir / "results" / "metrics" / "wanda_kmeans_k_sweep" / "wanda_kmeans_k_sweep_results.json"
    full_file = base_dir / "results" / "metrics" / "wanda_kmeans_k_sweep_full" / "wanda_kmeans_k_sweep_full_results.json"
    output_dir = base_dir / "results" / "visualizations" / "wanda_kmeans_k_sweep_full"
