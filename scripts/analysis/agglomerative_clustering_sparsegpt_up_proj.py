"""
Agglomerative clustering analysis on FULL feature width for SparseGPT up_proj matrices.
"""
...
def main():
    ...
    matrix_paths = sorted(data_dir.glob("layer*-mlp.up_proj.pt"))
    matrix_paths = sorted(matrix_paths, key=lambda p: int(p.stem.split('-')[0].replace('layer', '')))

    print(f"Found {len(matrix_paths)} up_proj matrices to analyze")
    ...
    for matrix_path in matrix_paths[:3]:
        ...
        output_path = results_dir / "agglomerative_clustering_up_proj_full_results.json"
        with open(output_path, 'w') as f:
            json.dump(all_results, f, indent=2)
        print(f"✓ Intermediate results saved to: {output_path}")

    print(f"\n{'='*60}")
    print(f"✓ FULL SparseGPT up_proj analysis complete!")
    print(f"{'='*60}")
