#!/usr/bin/env python3
"""
Visualize Wanda K-means K-Value Sweep Results

Creates a heatmap showing separation ratios across different k values
and projection types for Wanda pruning method.
"""
...
def create_separation_ratio_heatmap(results: dict, output_dir: Path):
    ...
    ax.set_title('Wanda: Separation Ratios Across K Values\n(Between-cluster / Within-cluster Hamming Distance)',
                fontsize=14, pad=20)
    ...
    # Save figure
    output_file = output_dir / 'wanda_k_sweep_heatmap.png'
...
def create_cluster_size_heatmap(results: dict, output_dir: Path):
    ...
    ax.set_xlabel('Number of Clusters (k)', fontsize=12)
    ax.set_ylabel('Projection Type', fontsize=12)
    ax.set_title('Wanda: Maximum Cluster Size Across K Values\n(Largest cluster out of 128 features)',
                fontsize=14, pad=20)
    ...
    # Save figure
    output_file = output_dir / 'wanda_k_sweep_cluster_sizes.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
...
def create_line_plot(results: dict, output_dir: Path):
    ...
    ax.set_title('Wanda: Separation Ratio vs K Value\n(Between-cluster / Within-cluster Distance)',
                fontsize=14, pad=20)
    ...
    # Save figure
    output_file = output_dir / 'wanda_k_sweep_line_plot.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
...
def main():
    """Generate all visualizations for Wanda k-sweep results."""
    # Define paths
    base_dir = Path(__file__).resolve().parent.parent.parent
    results_file = base_dir / "results" / "metrics" / "wanda_kmeans_k_sweep" / "wanda_kmeans_k_sweep_results.json"
    output_dir = base_dir / "results" / "visualizations" / "wanda_kmeans_k_sweep"
